...
interface TemplateEditorialOrangeProps {
...
export const TemplateEditorialOrange = forwardRef<TemplateRef, TemplateEditorialOrangeProps>(({
  keywords,
  seed = 0,
  onWordClick,
  onRenderComplete,
  ...
  const renderCloud = useCallback(async () => {
    ...
    const rng = seededRandom(seed + 404);
    ...
    const wordsData = sorted.map((kw, idx) => {
      ...
      if (isHero) {
        ...
        color = rng() < 0.4 ? "#c2410c" : PALETTE[Math.floor(rng() * PALETTE.length)];
        weight = "800";
      } else if (isMultiWord) {
    ...
    const layout = cloud<cloud.Word>()
      ...
      .on("end", (placedWords: cloud.Word[]) => {
        ...
        for (const w of placedWords) {
          ...
          const color = (w as { color?: string }).color || "#ea580c";
  ...
  return (
    <div ref={containerRef} className="w-full relative">
      <div className="relative rounded-2xl overflow-hidden bg-white border border-orange-500/20 shadow-[0_0_40px_rgba(234,88,12,0.1)]">
        <canvas
          ref={canvasRef}
...
TemplateEditorialOrange.displayName = "TemplateEditorialOrange";
