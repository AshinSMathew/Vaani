...
interface TemplateSlateTealProps {
...
export const TemplateSlateTeal = forwardRef<TemplateRef, TemplateSlateTealProps>(({
  keywords,
  seed = 0,
  onWordClick,
  onRenderComplete,
  ...
  const renderCloud = useCallback(async () => {
    ...
    const wordsData = sorted.map((kw, idx) => {
      ...
      if (isHero) {
        ...
        color = rng() < 0.35 ? "#0b1e2d" : PALETTE[Math.floor(rng() * PALETTE.length)];
        weight = "800";
      } else if (isMultiWord) {
    ...
    const layout = cloud<cloud.Word>()
      ...
      .on("end", (placedWords: cloud.Word[]) => {
        ...
        for (const w of placedWords) {
          ...
          const color = (w as { color?: string }).color || "#ffffff";
  ...
  return (
    <div ref={containerRef} className="w-full relative">
      <div className="relative rounded-2xl overflow-hidden bg-[#5c6e7a] border border-cyan-300/30 shadow-[0_0_40px_rgba(92,110,122,0.25)]">
        <canvas
          ref={canvasRef}
...
TemplateSlateTeal.displayName = "TemplateSlateTeal";
